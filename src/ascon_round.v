...
module ascon_round (
    ...
    // Convention: x0 = state[319:256], x1 = state[255:192], ..., x4 = state[63:0]
    wire [63:0] x0_in, x1_in, x2_in, x3_in, x4_in;
    assign x0_in = state_in[319:256];
    assign x1_in = state_in[255:192];
    assign x2_in = state_in[191:128];
    assign x3_in = state_in[127:64];
    assign x4_in = state_in[63:0];
    ...
    // ---- pS: nonlinear S-box layer ----
    // The ASCON S-box is a 5-bit S-box applied across all 64 bit-positions.
    // It can be expressed as a sequence of XOR and AND operations on the
    // five state words. This is the standard bitsliced form.
    //
    //   x0 ^= x4
    //   x4 ^= x3
    //   x2 ^= x1
    ...
    //   x3 ^= x2
    //   x2 = ~x2
    wire [63:0] s0_a, s1_a, s2_a, s3_a, s4_a;
    assign s0_a = x0_c ^ x4_c;
    assign s4_a = x4_c ^ x3_c;
    assign s2_a = x2_c ^ x1_c;
    // Other two pass through unchanged at this step
    ...
    // ---- pL: linear diffusion (rotation mixing) ----
    // Each word gets XORed with two rotated versions of itself:
    ...
    assign x0_l = s0_c ^ rotr64(s0_c, 19) ^ rotr64(s0_c, 28);
    assign x1_l = s1_c ^ rotr64(s1_c, 61) ^ rotr64(s1_c, 39);
    assign x2_l = s2_c ^ rotr64(s2_c,  1) ^ rotr64(s2_c,  6);
    assign x3_l = s3_c ^ rotr64(s3_c, 10) ^ rotr64(s3_c, 17);
    assign x4_l = s4_c ^ rotr64(s4_c,  7) ^ rotr64(s4_c, 41);
